<template>
  <div class="galaxy-container" ref="containerRef">
    <div class="stats-bar">
      <div class="stat"><span class="stat-num">{{ checkinUsers.length }}</span> 人已签到</div>
...
const props = defineProps<{ checkinUsers: any[]; event: any }>();

const containerRef = ref<HTMLDivElement>();
let scene: THREE.Scene, camera: THREE.PerspectiveCamera, renderer: THREE.WebGLRenderer;
let animId: number, clock: THREE.Clock;

let galaxyGroup: THREE.Group;
let bulgePoints: THREE.Points;
let diskPoints: THREE.Points;
let haloPoints: THREE.Points;
let coreGlow: THREE.Group;
...
const createGalaxy = () => {
  ...
  // ── 1. Central bulge (spherical gaussian) ──
  const B_N = 10000;
...
const createBgStars = () => {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const cl = new THREE.Color().setHSL(rand(0.55, 0.7), rand(0.1, 0.5), rand(0.3, 1.0));
  ...
  bgStars = new THREE.Points(g, new THREE.PointsMaterial({
    size: 0.13, vertexColors: true, blending: THREE.AdditiveBlending,
...
const init = () => {
  ...
  const l = new THREE.PointLight(0xffcc80, 8, 45, 2); l.position.set(0, 0, 0); scene.add(l);
...
const animate = () => {
  ...
  if (coreGlow) coreGlow.scale.setScalar(1 + Math.sin(performance.now() * 0.002) * 0.05);
  ...
  // camera
  const cx = Math.sin(T * 0.3) * 9;
  const cz = 28 + Math.cos(T * 0.25) * 5;
  const cy = 22 + Math.sin(T * 0.22) * 3;
  camera.position.lerp(new THREE.Vector3(cx, cy, cz), dt * 0.4);
  camera.lookAt(0, 0, 0);
  ...
  userM.forEach((e, uid) => {
    ...
    if (e.fProg < 1) {
      ...
      e.ang += e.speed * dt;
      const ox = Math.cos(e.ang) * e.rad;
      const oz = Math.sin(e.ang) * e.rad;
      const oy = e.oY + Math.sin(T * 3.5 + ox) * 0.2;
...
<style scoped>
.galaxy-container { width: 100%; height: 100%; position: absolute; inset: 0; z-index: 1; }
.stats-bar {
  ...
  border: 1px solid rgba(255,255,255,0.07); border-radius: 50px; padding: 12px 32px;
}
.stat { font-size: 18px; color: rgba(255,255,255,0.82); font-weight: 500; }
.stat-num { font-size: 28px; font-weight: 700; color: #ffd54f; margin-right: 4px; }
.stat-hint { font-size: 14px; color: rgba(255,255,255,0.3); max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
</style>